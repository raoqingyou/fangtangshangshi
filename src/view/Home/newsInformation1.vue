  ...
  <div id="policyGuidelines">
        <main class="page-content ant-layout-content">
          <div class="policy-guidelines-wrap">
            <div class="policy-guidelines-title">我要保险</div>
            <div class="policy-table-wrap">
              <div class="policy-table">
                ...
                <span class="policy-table-total">共{{pagination.total || 0}}条记录</span>
              </div>
            </div>
          </div>
        </main>
...
export default {
  ...
  mounted () {
    this.getNewsList({channel_id: 30,page:1})
  },
  methods: {
    handleCurrentChange (val) {
      this.getNewsList({channel_id: 30,page:val})
    },
    getNewsList(data) {
    this.$axios({  //机构列表
      method: 'POST',
      url: '/api/index/getNewsListByid',
      data: data
    }).then(res=>{
      let { data } = res.data
