<template>
  <div id="examine" class="examineContent">
    <div class="details">
      <ul class="details-ul">
        ...
        <li v-for="(item, index) in tableData" :key="index">
          <div class="enterprise">
            <div class="liName">
              ...
              ><span class="liState">审批</span>
            </div>
            <div class="btn">
              <div class="btnPass" @click="open(item)"><span>返款</span></div>
...
export default {
  ...
  methods: {
    ...
    ok() {
      this.$store.state.callBack = 2
      let token = this.$store.state.token;
      if (this.value.show) {
        this.changeapply(token, this.value.id, 4);
      } else {
        this.changeapply(token, this.value.id, 3);
      }
    },
